# Portfolio  
**現場の課題をITで仕組み化し、業務改善を実現するエンジニアを目指しています**  
  
不便を放置せず、AIやIoTを組み合わせて「解決する仕組み」を作るのが好きです。  

  ---
  
## 🍄‍🟫しいたけ栽培支援モニタリングシステム  
**24時間の監視と遠隔制御で、栽培の失敗を防ぐIoT基盤**  
温湿度センサーとマイコンを組み合わせ、異常時に通知やエアコン制御を行うシステムです。  

**技術**  
- **Hardware**：ESP8266 / DHT22
- **Language**：C++（Arduino IDE）/ Google Apps Script
- **Integrations**：Discord Webhook / Nature Remo

[🔗Repository](https://github.com/h-kanno-dev/shiitake_sensor)  

  ---
  
## 📅dAIly（デイリー）- AI搭載カレンダー登録＆リマインドLINE Bot
**曖昧な話し言葉を、Gemini APIで正確なスケジュールへ**  
LINEで送った文章から日時・場所・詳細などを自動抽出し、カレンダー登録とリマインドを行うLINE Botです。  

**技術**  
- **AI**：Gemini Flash（Latest）
- **Platform**：Google Apps Script
- **Integrations**：LINE Messaging API / Google Calendar API

[🔗Repository](https://github.com/h-kanno-dev/dAIly)  

  ---
  
## 🛠️Skills & Tools
### Language & Platform  
- **Google Apps Script**：現在のメイン開発環境。日常の自動化やAPI連携のメインツールとして活用。
- **Python**：LINE Bot開発の経験あり。Render運用時に発生していた手動での認証更新を簡略化するため、現在のGASへ移行。
- **Java / PHP / SQL**：職業訓練にてWebアプリのCRUDを習得。エラーと向き合いながらも一貫した開発工程を経験。
- **C++（Arduino IDE）**：ESP8266による電子工作の実装に使用。ハードウェアとソフトの連携を経験。
### AI Pairing Development
- **Gemini / ChatGPT**：開発プロセス全般（設計・デバッグ・リサーチ）に活用。AIの提示するコードを鵜呑みにせず、一つひとつの挙動と根拠を自分なりに検証しながら完成まで進める手法を採っています。未知の技術領域においても、AIを「家庭教師」として対話を重ね、自力で解決しきるまでの補助ツールとして取り入れています。
